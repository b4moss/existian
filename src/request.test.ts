...
describe("createCheckRunner", () => {
  ...
  it("does not apply results from a superseded generation", async () => {
    ...
    runner.schedule("old");
    await Promise.resolve();
    // Time out the first request (bumps handling via abort + timeout result).
    ...
    expect(fetchMock).toHaveBeenCalledTimes(2);
    // Late success for the timed-out generation must be ignored.
    resolveFirst(new Response(JSON.stringify({ from: "old" }), { status: 200 }));
    for (let i = 0; i < 10; i++) await Promise.resolve();
    const okCalls = onResult.mock.calls.filter((c) => c[0].ok);
    expect(okCalls.every((c) => (c[0] as { body?: { from?: string } }).body?.from === "second")).toBe(
      true,
    );
    expect(
      okCalls.some((c) => (c[0] as { body?: { from?: string } }).body?.from === "old"),
    ).toBe(false);
  });
