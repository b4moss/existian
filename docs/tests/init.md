...
- 追加オプション: `buildRequest?`、`fetch?`。詳細は [hooks.md](./hooks.md)
- v0.1 の宣言的パスと既定送信規約は維持する（フック未使用時に破壊的変更を入れない）
- グローバルに「最後の init だけが有効」にはしない。複数 `init` はそれぞれ独立した Handle を持つ（同一要素は最初に bind した側が保持し、他方はスキップ）
